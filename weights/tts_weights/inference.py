...
class PerturbationGradTTS():
    def __init__(self, model_path, hifi_config_path, hifi_path, spk_emb_dim=64, n_enc_channels=192, filter_channels=768,
        ...
        self.generator = GradTTS(len(symbols)+1, 1, spk_emb_dim, n_enc_channels, filter_channels,
    ...
    def set_hifigan(self, config_path, checkpts_path):
        ...
        hifi = HiFiGAN(h)
        hifi.load_state_dict(torch.load(checkpts_path, map_location=lambda loc, storage: loc)['generator'])
        _ = hifi.to(self.device).eval()
